...
public class TimeseriesAnalyticalStoreTest {
    ...
    public void testCreateTable() {
        ...
        Assert.assertEquals(86400 * 30, analyticalStores.get(0).getTimeToLive());
        Assert.assertEquals(AnalyticalStoreSyncType.SYNC_TYPE_FULL, analyticalStores.get(0).getSyncOption());
    }
    ...
    public void testDescribeIndexSyncPhase() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            row.addField("usage", ColumnValue.fromLong(i));
            putRequest.addRow(row);
        }
        client.putTimeseriesData(putRequest);
        ...
        Assert.assertEquals(1, indexMetas.size());
        Assert.assertEquals(SyncStat.SyncPhase.FULL, indexMetas.get(0).getIndexSyncPhase());
    }
}
