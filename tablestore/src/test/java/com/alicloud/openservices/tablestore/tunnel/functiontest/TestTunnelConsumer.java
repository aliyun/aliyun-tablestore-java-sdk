...
public class TestTunnelConsumer {
    ...
    public void testFuzzyConsume() throws Exception {
        // 1. Randomly write a certain amount of data
        ...
        // 2. Create a full-quantity type Tunnel to start consumption, and the Tunnel enters the incremental state when the consumption is completed.
        ...
        // 3. Wait for data consumption to complete
    ...
    public void testMultiTimesReadConsume() throws Exception {
        ...
        // 2. Create a full-quantity type Tunnel to start consumption, and the Tunnel enters the incremental state when the consumption is complete.
        ...
        // 3. Wait for data consumption to finish
        DescribeTunnelResponse describeTunnelResponse = tunnelClient.describeTunnel(
                new DescribeTunnelRequest(TABLE_NAME, TUNNEL_NAME));
        while (describeTunnelResponse.getTunnelInfo().getStage() != TunnelStage.ProcessStream) {
            Thread.sleep(1000);
    ...
    public void testConsumeOriginColumn() throws Exception {
        String column = "col1";
        //1. Set the original column for the table
        updateTableWithSetOriginColumn(syncClient, column);
        Random rand = new Random(System.currentTimeMillis());

        // 2. Create a Tunnel of incremental type to start consuming, and assert after the Tunnel reads out the data.
        workerConfig.setReadMaxTimesPerRound(rand.nextInt(10) + 1);
        workerConfig.setReadMaxBytesPerRound((rand.nextInt(50) + 4) * 1024 * 1024);
        workerConfig.setHeartbeatIntervalInSec(10);
        ...
        // 3. Write a piece of data and then update it
        putAndUpdateRow(syncClient, column);

        // 4. Wait for data consumption to finish
