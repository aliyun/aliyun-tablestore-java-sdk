...
public class APITest {
    ...
    /**
     * Create table with MaxVersion=3, call UpdateRow 100 times, each time writing 128 different columns, and each column contains one version.
     * Call GetRow each time specifying different column names, reading 128 columns each time, and data verification passes. This CASE should be performed 
     * for column values of BINARY/INTEGER/STRING/BOOLEAN/DOUBLE types.
    ...
    public void testCase1() throws UnsupportedEncodingException {
        ...
        // Create table with MaxVersion=3
        ...
        int columnIndex = 0;
        for (int p = 0; p < 100; p++) { // 100 times
            puts.clear();
            // Write 128 different columns each time
            for (int i = 0; i < 128; i++) {
    ...
    /**
     * Create table with MaxVersion=100000, UpdateRow repeated 100 times, writing 1 column each time, 
     * and each column contains 128 new versions. GetRow specifies a different version each time, 
     * reading 128 versions each time, and the data verification passes. This CASE should be run for 
     * BINARY/INTEGER/STRING/BOOLEAN/DOUBLE typed column values.
     * @throws UnsupportedEncodingException
     */
    @Test
    public void testCase2() throws UnsupportedEncodingException {
        ...
        // Create table with MaxVersion=100000
        ...
        for (int p = 0; p < 100; p++) { // 100 times
            puts.clear();
            // Write 1 different column each time
            for (int i = 0; i < 1; i++) {
                String columnName = String.format("attr_%06d", columnIndex++);
    ...
    /**
     * MaxVersion=100, PutRow includes 16 columns, column name length is 255, each column contains 8 cells with the same timestamp.
     * GetRow reads and verifies that the last data of each cell is returned. Then UpdateRow is performed with the same row, columns, and timestamp.
     * GetRow reads and verifies again.
     */
    @Test
    public void testCase5() {
        ...
        // Then perform UpdateRow, including the same row, column, and timestamp, and then use GetRow to read and verify again.
    ...
    /**
     * MaxVersion=100, BatchWriteRow includes 10 rows, each row contains 16 columns, the column name length is 255, 
     * each column contains 8 cells with the same timestamp. Operations are performed in the order of put, update, and then delete, 
     * and BatchGetRow is used to read and verify each time.
     */
    ...
    public void testCase6() {
        ...
        // BatchWriteRow includes 10 rows, each row contains 16 columns, the column name length is 255, each column contains 8 cells with the same timestamp.
        {
            for (int k = 0; k < 10; k++) {
                List<Column> columns = new ArrayList<Column>();
                PrimaryKey pk = PrimaryKeyBuilder.createPrimaryKeyBuilder()
