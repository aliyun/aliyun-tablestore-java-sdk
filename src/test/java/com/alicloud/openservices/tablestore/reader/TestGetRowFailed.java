...
/**
 * 测试case
 * testPartitionFailedRead : BatchGetRow读取数据出现PartitionFailed，reader层不会进行重试
 * testTableStoreExceptionFailedRead : BatchGetRow读取数据出现TableStoreException，则会拆分为单行请求进行GetRow重试；GetRow再失败后则不会重试
 * testClientExceptionFailedRead : BatchGetRow读取数据出现ClientException，reader层不会进行重试
 */
public class TestGetRowFailed {
    ...
    public void testPartitionFailedRead() throws InterruptedException {
        ...
        assertEquals(succeedRows.get(), 5000);
        assertEquals(failedRows.get(), 5000);
    ...
    public void testClientExceptionFailedRead() throws InterruptedException {
        ...
        assertEquals(succeedRows.get(), 0);
        assertEquals(failedRows.get(), 10000);
