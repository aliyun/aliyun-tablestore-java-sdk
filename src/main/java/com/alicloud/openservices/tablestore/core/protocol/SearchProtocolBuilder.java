...
public class SearchProtocolBuilder {
    ...
    public static Search.VectorOptions buildVectorOptions(VectorOptions vectorOptions) {
        ...
        if (vectorOptions.getIndexParameter() != null) {
            VectorIndexParameter indexParameter = vectorOptions.getIndexParameter();
            if (indexParameter instanceof HNSWIndexParameter) {
                HNSWIndexParameter hnswIndexParameter = (HNSWIndexParameter) indexParameter;
                Search.HNSWIndexParameter.Builder hnswBuilder = Search.HNSWIndexParameter.newBuilder();
                if (hnswIndexParameter.getM() != null) {
                    hnswBuilder.setM(hnswIndexParameter.getM());
                }
                if (hnswIndexParameter.getEfConstruction() != null) {
                    hnswBuilder.setEfConstruction(hnswIndexParameter.getEfConstruction());
                }
                builder.setIndexType(Search.VectorIndexType.VI_HNSW);
                builder.setIndexParameter(hnswBuilder.build().toByteString());
            } else {
                throw new IllegalArgumentException("unknown index parameter implement:" + indexParameter.getClass().getSimpleName());
            } 
        }
        return builder.build();
    }
